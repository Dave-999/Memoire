...
However, knowing the composition is not sufficient to predict the microstructure of the fabricated alloy. Indeed, other parameters have to be taken into account. As shown in Fig. \ref{fig:micro_cast}, casting result in lamellar precipitates of Si-phase, measuring several tens of micrometers, floating in an Al matrix. But changing the manufacturing process of the part can completely transform the resulting microstructure. In Fig. \ref{fig:micro_am}, we can ----------

\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.35]{Images/micro_cast}
	\decoRule
	\caption[Common microstructure of an Al-Si cast alloy]{Common microstructure of an Al-Si cast alloy (from Wang et al., 2012 \parencite{Wang12}).}
...
\begin{figure}[ht]
	...
	\decoRule
	\caption[Microstructure of an Al-Si alloy manufactured by SLM at different scales. Locations of pictures (a), (b) and (c) appear on the top left picture.]{[Microstructure of an Al-Si alloy manufactured by SLM at different scales. Locations of pictures (a), (b) and (c) appear on the top left picture (from Thijs et al., 2012 \parencite{Thijs13}).}
	\label{fig:micro_am}
\end{figure}

The difference in mechanical properties between cast and AM parts lies in the very specific microstructure formed by SLM, as shown in Fig. \ref{label}
