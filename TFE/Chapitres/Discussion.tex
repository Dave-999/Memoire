...
Kempen et al. found optimal parameters values that are not far to the ones of this work (see table \ref{tab:compKemp}) \parencite{Kempen110817}. The energy density is however significantly bigger, especially since the laser was more focused in this work (which is not taken into account for the approximate computation). $E_d$ is rather close to the optimal value obtained by Read et al. (60 $\frac{J}{mm^3}$) \parencite{Read150417}. \\
...
A relative density of 99.90 [\%] was reached, which is superior to the best values found in the literature \parencite{EOS}. It is likely that all samples fabricated with the optimised parameters had $\rho_{rel}>99.4[\%]$ according to the measurements. Results depended on the powder age and recycling ([REF]). It can thus be confirmed that the parameters chosen allowed for a right energy input and a correct melt pools overlapping. The hardness was significantly bigger than what is typically expected - approximately 10 [HV] more (see section \ref{MMABMP}). [EXPLICATIONS?= THERMIQUE? OU CONTRAINTES RESIDUELLES?]\\

However, one should note that the optimisation of the overall density was perhaps not the most pertinent to conduct. [TAILLE MAX DES PORES, CONTRAINTES RESIDUELLES ...]\\ 
