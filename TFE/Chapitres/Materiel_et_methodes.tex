...
The traction tests were performed on a \textit{RetroLine testControl II} electro-mechanical testing system manufactured by \textit{Zwick Roell}. It works in the following manner:

\begin{itemize}
\item The tested sample is clamped with grips on its extremities. Both of them are fixed to a cross head (see figure \ref{fig:Trac}).
\item A contact extensometer is placed on the sample. Its purpose is to measure the length change during the test. 
\item Parameters are selected thanks to a software connected to the machine: the extensometer initial gauge length, the cross head speed and the pre load. The role of this initial load is to limit the backlash in the assembly. 
\item The lower cross head goes down. This induces a force and a displacement, both of which are saved in an \textit{Excel} file. The test goes on until the fracture of the specimen occurs, unless if it is stopped beforehand.
\end{itemize}
...
A tensile test can be used to compute multiple properties. \\textcolor{gray}{[Calcul des charactéristique matériau]}
